...
interface InnerPagesTemplateProps {
    platform: 'wechat' | 'xhs';
    bgImage: string;
    themeText: string;
    images: string[];
...
export const InnerPagesTemplate = forwardRef<HTMLDivElement, InnerPagesTemplateProps>(({
    platform,
    bgImage,
    themeText,
    images,
    ...
    return (
        ...
        >
            ...
            {/* Image Container — takes up most of the card, stops before compact text area */}
            <div className="absolute top-6 left-6 right-6 bottom-[18%] z-20 pointer-events-none overflow-hidden">
                <div className="w-full h-full flex flex-col justify-evenly items-center">
                    {images.length === 0 ? (
                        <>
                            <div className="h-[46%] aspect-video max-w-full text-white/50 text-sm border-2 border-dashed border-white/20 rounded-lg flex items-center justify-center bg-white/5 backdrop-blur-sm">
                                Page 1
                            </div>
                            <div className="h-[46%] aspect-video max-w-full text-white/50 text-sm border-2 border-dashed border-white/20 rounded-lg flex items-center justify-center bg-white/5 backdrop-blur-sm">
                                Page 2
                        ...
                        <div className="h-[94%] max-w-full shadow-[0_20px_60px_-15px_rgba(0,0,0,0.8)] rounded-lg overflow-hidden border-3 border-white bg-white flex items-center justify-center">
                            <img src={images[0]} alt="Inner Page 1" className="max-w-full max-h-full object-contain" />
                        </div>
                    ) : (
                        <>
                            {images.slice(0, 2).map((img, i) => (
                                <div key={i} className="h-[46%] max-w-full shadow-[0_10px_40px_-10px_rgba(0,0,0,0.7)] rounded-lg overflow-hidden border-2 border-white bg-white flex items-center justify-center">
                                    <img src={img} alt={`Inner Page ${i + 1}`} className="max-w-full max-h-full object-contain" />
                                </div>
                            ))}
                        </>
                    )}
            ...
            <div className="absolute bottom-0 left-0 right-0 z-30 px-7 pb-7 pt-4 text-white pointer-events-none bg-gradient-to-t from-slate-950/90 via-slate-900/50 to-transparent">
                <div className="flex items-center gap-2 opacity-90 mb-1">
                    ...
                    <span className="font-bold tracking-widest text-[9px] uppercase text-emerald-50">{subText}</span>
                </div>

                <h2 className={`${titleClass} font-extrabold leading-tight tracking-tight drop-shadow-md whitespace-nowrap overflow-hidden text-ellipsis`}>
                    {themeText}
                </h2>

                <div className="w-7 h-0.5 bg-emerald-500 rounded-full mt-1.5" />
            </div>
        </div >
    );
});

InnerPagesTemplate.displayName = 'InnerPagesTemplate';

...
async function reviewCurriculum(cfg) {
    let content;
    try {
        content = readFileSync(`${DIR}/01-curricula/${cfg.file}`, 'utf8');
    } catch (e) {
        console.error(`  ⚠️ Skipping ${cfg.file} because it could not be read (generation likely failed).`);
        return `Review skipped: Generation for ${cfg.file} failed.`;
    }
...
- Is 180 minutes realistic for the described activities?
Score: [1-10] and explain.
